...
document.addEventListener('DOMContentLoaded', function (event) {
    if (typeof calendarData !== 'undefined') {
        ...
        const isMobile = () => {
            ...
            return window.matchMedia(`(max-device-${isPortrait ? 'width' : 'height'}: ${480}px)`)
                .matches;
        };
        ...
        const updateStartSelect = () => {
            const sameDay =
                jQuery('div.repetition-start span.date').text() ===
                jQuery('div.repetition-end span.date').text();
        ...
        const countOverbookedDays = (start, end) => {
            ...
            for (let day in globalCalendarData['days']) {
                ...
                if (
                    ...
                    if (
                        globalCalendarData['days'][day]['holiday'] ||
                        globalCalendarData['days'][day]['locked']
                    ) {
                        overbookedDays++;
                    }
                }
        ...
        const initPicker = () => {
            picker = new Litepicker({
                ...
                startDate: moment().isAfter(globalCalendarData['startDate'])
                    ? moment().format('YYYY-MM-DD')
                    : globalCalendarData['startDate'],
                scrollToDate: true,
                inlineMode: true,
                firstDay: 1,
                ...
                onChangeMonth: function (date, idx) {
                    ...
                    const calStartDate = moment(date.format('YYYY-MM-DD'))
                        .date(0)
                        .format('YYYY-MM-DD');
                    // first day of next month selection
                    const calEndDate = moment(date.format('YYYY-MM-DD'))
        ...
        const updatePicker = (globalCalendarData) => {
            fadeOutCalendar();
            picker.setOptions({
                minDate: moment().isAfter(globalCalendarData['startDate'])
                    ? moment().format('YYYY-MM-DD')
                    : globalCalendarData['startDate'],
