...
use CommonsBooking\Eluceo\iCal\Domain\Entity\Calendar;
use CommonsBooking\Eluceo\iCal\Domain\ValueObject\MultiDay;
use CommonsBooking\Eluceo\iCal\Domain\ValueObject\SingleDay;
use CommonsBooking\Eluceo\iCal\Domain\ValueObject\TimeSpan;
use CommonsBooking\Eluceo\iCal\Presentation\Factory\CalendarFactory;
use CommonsBooking\Eluceo\iCal\Domain\ValueObject\DateTime;
use CommonsBooking\Eluceo\iCal\Domain\ValueObject\Timestamp;
use CommonsBooking\Eluceo\iCal\Domain\Enum\EventStatus;
use CommonsBooking\Eluceo\iCal\Domain\ValueObject\Location;
use CommonsBooking\Eluceo\iCal\Domain\ValueObject\UniqueIdentifier;
use CommonsBooking\Eluceo\iCal\Domain\Entity\Event;
use CommonsBooking\Eluceo\iCal\Domain\ValueObject\Date;
use CommonsBooking\Eluceo\iCal\Domain\ValueObject\GeographicPosition;
...
class iCalendar {
	...
	public function addBookingEvent(
		...
		if ( $php_date_time_zone->getTransitions() ) {
			$timezone = \CommonsBooking\Eluceo\iCal\Domain\Entity\TimeZone::createFromPhpDateTimeZone(
				$php_date_time_zone,
		...
		if ( $booking->isFullDay() ) {
			...
			$occurrence = new TimeSpan(
				new \CommonsBooking\Eluceo\iCal\Domain\ValueObject\DateTime( $booking_startDateDateTime, true ),
				new \CommonsBooking\Eluceo\iCal\Domain\ValueObject\DateTime( $booking_endDateDateTime, true )
			);
		}
